...
switch ($action) {
    ...
    case 'recent': {
        ...
        $latest = one($db,
          "SELECT Sci_Name AS sci, Com_Name AS com, Date||' '||Time AS last_seen, "
        . "       CAST(MAX(0, (julianday('now','localtime') - julianday(Date||' '||Time)) * 86400) AS INTEGER) AS age_seconds "
        . "FROM detections ORDER BY Date DESC, Time DESC LIMIT 1"
        );
        echo json_encode(['hours' => $hours, 'from' => $hasExactStart ? $from : null, 'species' => $rs, 'latest' => $latest, 'as_of' => date('c')]);
        break;
    }
