(function () {
  ...
  function applySiteName(value) {
    var name = String(value || '').trim() || 'your birds';
    var label = document.getElementById('aboutLink');
    if (label) label.textContent = name;
    document.title = name + ' · BirdNET';
  }
  ...
  function renderCollage(items) {
    ...
    placed.forEach(function (r) {
      ...
      // Keep the first-of-season ribbon in proportion to this bird rather
      // than the viewport. A floor preserves the numeral on tiny newcomers.
      var seasonBadgeScale = Math.max(0.46, Math.min(1, Math.min(r.fullW, r.fullH) / 120));
      btn.style.setProperty('--season-first-scale', seasonBadgeScale.toFixed(3));
  ...
  function latestDetection() {
    if (DATA.recent && DATA.recent.latest && DATA.recent.latest.last_seen) return DATA.recent.latest;
    var candidates = [];
    if (DATA.recent && Array.isArray(DATA.recent.species)) candidates = candidates.concat(DATA.recent.species);
    if (DATA.lifelist && Array.isArray(DATA.lifelist.species)) candidates = candidates.concat(DATA.lifelist.species);
    var latest = null;
    candidates.forEach(function (row) {
      if (!row || !row.last_seen || (latest && row.last_seen <= latest.last_seen)) return;
      latest = row;
    });
    return latest;
  }

  function detectionTimestamp(value, asOf) {
    if (!value) return NaN;
    var offset = '';
    var match = String(asOf || '').match(/(Z|[+-]\d\d:\d\d)$/);
    if (match) offset = match[1];
    return Date.parse(String(value).replace(' ', 'T') + offset);
  }

  function recentAgeLabel(totalSeconds) {
    var seconds = Math.max(0, Math.floor(totalSeconds));
    if (seconds < 60) return seconds + ' seconds ago';
    var minutes = Math.floor(seconds / 60);
    var remainder = seconds % 60;
    return minutes + ' ' + (minutes === 1 ? 'minute' : 'minutes') + ' ' + remainder + ' seconds ago';
  }

  function updateRecentHeard() {
    var bird = document.getElementById('recentHeardBird');
    var age = document.getElementById('recentHeardAge');
    if (!bird || !age) return;
    var latest = latestDetection();
    if (!latest) {
      bird.textContent = 'waiting for a bird';
      age.textContent = '--';
      return;
    }
    var asOf = (DATA.recent && DATA.recent.as_of) || (DATA.lifelist && DATA.lifelist.as_of) || '';
    bird.textContent = latest.com || latest.sci || 'unknown bird';
    var baseAge = Number(latest.age_seconds);
    if (Number.isFinite(baseAge)) {
      var responseTime = Date.parse(asOf);
      var elapsed = Number.isFinite(responseTime) ? Math.max(0, Math.floor((Date.now() - responseTime) / 1000)) : 0;
      age.textContent = recentAgeLabel(Math.floor(baseAge) + elapsed);
      return;
    }
    var timestamp = detectionTimestamp(latest.last_seen, asOf);
    age.textContent = Number.isFinite(timestamp) ? recentAgeLabel((Date.now() - timestamp) / 1000) : 'heard recently';
  }
  setInterval(updateRecentHeard, 1000);
  ...
  function renderMenu(menu) {
    ...
    items.innerHTML =
      ...
      + '<button type="button" class="ui-theme-btn" id="uiThemeBtn" aria-pressed="false">'
      + '  <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.2 15.4A8.7 8.7 0 0 1 8.6 3.8 8.8 8.8 0 1 0 20.2 15.4Z"/></svg>'
      + '  <span>dark mode</span><strong id="uiThemeState">off</strong>'
      + '</button>'
      + '<div class="mode-switch" id="modeSwitch">'
      + '  <div class="mode-switch-head"><span>microphone owner</span><strong id="modeSwitchState">checking...</strong></div>'
      + '  <div class="mode-switch-buttons">'
      + '    <button type="button" data-mode="pi">BirdNET-Pi</button>'
      + '    <button type="button" data-mode="bng">BNG</button>'
      + '  </div>'
      + '  <small id="modeSwitchHint">Only one analyzer controls the microphone at a time.</small>'
      + '</div>'
    ...
    var uiThemeBtn = document.getElementById('uiThemeBtn');
    var uiThemeState = document.getElementById('uiThemeState');
    function updateUiThemeButton() {
      var dark = document.documentElement.dataset.uiTheme === 'dark';
      if (uiThemeBtn) uiThemeBtn.setAttribute('aria-pressed', dark ? 'true' : 'false');
      if (uiThemeState) uiThemeState.textContent = dark ? 'on' : 'off';
    }
    if (uiThemeBtn) uiThemeBtn.addEventListener('click', function () {
      var dark = document.documentElement.dataset.uiTheme !== 'dark';
      document.documentElement.dataset.uiTheme = dark ? 'dark' : 'light';
      try { localStorage.setItem('avian-ui-theme', dark ? 'dark' : 'light'); } catch (e) {}
      updateUiThemeButton();
    });
    updateUiThemeButton();

    var modeSwitch = document.getElementById('modeSwitch');
    var modeState = document.getElementById('modeSwitchState');
    function paintMode(mode) {
      if (!modeState) return;
      modeState.textContent = mode === 'pi' ? 'BirdNET-Pi active' : mode === 'bng' ? 'BNG active' : mode === 'mixed' ? 'mixed / check services' : 'unavailable';
      if (modeSwitch) modeSwitch.setAttribute('data-mode', mode);
    }
    function loadMode() {
      fetch(apiUrl('mode-switch.php'), { credentials: 'same-origin', cache: 'no-store' })
        .then(function (r) { return r.json(); }).then(function (j) { paintMode(j.mode); }).catch(function () { paintMode('unknown'); });
    }
    if (modeSwitch) {
      modeSwitch.querySelectorAll('button[data-mode]').forEach(function (button) {
        button.addEventListener('click', function () {
          var target = button.dataset.mode;
          if (!window.confirm('Switch microphone control to ' + (target === 'pi' ? 'BirdNET-Pi' : 'BNG') + '?')) return;
          modeState.textContent = 'switching...';
          modeSwitch.querySelectorAll('button').forEach(function (b) { b.disabled = true; });
          fetch(apiUrl('mode-switch.php'), { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'mode=' + encodeURIComponent(target) })
            .then(function (r) { return r.json().then(function (j) { if (!r.ok || !j.ok) throw new Error(j.error || 'switch failed'); return j; }); })
            .then(function (j) { paintMode(j.mode); setStatus((target === 'pi' ? 'BirdNET-Pi' : 'BNG') + ' now owns the microphone.', false); })
            .catch(function (e) { modeState.textContent = e.message || 'switch failed'; })
            .finally(function () { modeSwitch.querySelectorAll('button').forEach(function (b) { b.disabled = false; }); });
        });
      });
      loadMode();
    }
  ...
  function loadSettings() {
    fetch(apiUrl('config.php'), { credentials: 'same-origin', cache: 'no-store' })
      ...
      .then(function (cfg) {
        ...
        var html = ''
          + settingsText('SITE_NAME', 'Collage location name', 'shown above Heard Recently', v.SITE_NAME || '', 'your birds')
  ...
  function settingsText(key, label, hint, val, placeholder) {
    return ''
      ...
      + '  </div>'
      + '  <div class="secret-control">'
      + '    <input type="text" autocomplete="off" spellcheck="false" placeholder="' + attrEsc(placeholder || 'US-MO') + '" value="' + attrEsc(val || '') + '" data-key="' + key + '">'
      + '  </div>'
      + '</div>';
  }
  ...
  function saveSettings() {
    ...
    var siteNameChanged = Object.prototype.hasOwnProperty.call(pending, 'SITE_NAME');
    ...
    var nextSiteName = siteNameChanged ? pending.SITE_NAME : null;
    ...
    fetch(apiUrl('config.php'), {
      ...
      .then(function (res) {
        if (res.ok && res.j.ok) {
          ...
          if (siteNameChanged) applySiteName(nextSiteName);
          if (artChanged) {
